...
class ReportTests(unittest.TestCase):
    ...
    def test_parameterized_reasons_group_without_dropping_exact_text(self):
        session = _session(self.root, "reasons")
        session.append("decisions", {"action": "WAIT", "reason": "size_too_small($0.00)", "asset": "BTC"})
        session.append("decisions", {"action": "WAIT", "reason": "size_too_small($4.20)", "asset": "ETH"})
        session.append("decisions", {"action": "WAIT", "reason": "entry_too_cheap(0.010<0.02)", "asset": "SOL"})
        session.close()
        report = analyze(session.directory, now=WHEN)
        groups = dict(report["decisions"]["reason_groups"])
        self.assertEqual(groups["size_too_small"], 2)
        self.assertEqual(groups["entry_too_cheap"], 1)
        self.assertEqual(report["decisions"]["reasons"]["size_too_small($4.20)"], 1)
        self.assertEqual(report["decisions"]["reason_table"][0]["reason"], "size_too_small")
        session = _session(self.root, "buy-exits")
        session.append("decisions", {"action": "BUY_YES", "reason": "OK", "asset": "BTC", "decision_id": "a"})
        session.append("decisions", {"action": "BUY_NO", "reason": "OK", "asset": "ETH", "decision_id": "b"})
        session.append("operational_events", {"event": "entry_identity_unavailable", "asset": "BTC", "decision_id": "a"})
        session.append("operational_events", {"event": "entry_strategy_unavailable", "asset": "ETH", "decision_id": "b"})
        session.close()
        report = analyze(session.directory, now=WHEN)
        self.assertEqual(report["funnel"]["action_signals"], 2)
        self.assertEqual(report["funnel"]["intended"], 0)
        self.assertEqual(report["funnel"]["advisory_rejections"]["entry_identity_unavailable"], 1)
        self.assertEqual(report["funnel"]["advisory_rejections"]["entry_strategy_unavailable"], 1)
